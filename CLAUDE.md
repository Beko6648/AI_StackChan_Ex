...
**重要な認識:**
`record_number` を小さくする（例: 400→200）のは VAD ではない。VAD の定義は「発話の終わりを検出して録音を止める」ことであり、固定時間の短縮では長い発話が切れるリスクがあり本質的な解決にならない。現在の「ポスト処理 VAD」は発話終了の検出は正確に行うが録音時間は短縮できない。「末尾トリム」として運用するのが正確な表現。録音時間の短縮には `i2s_read()` 直接使用が唯一の正解。
