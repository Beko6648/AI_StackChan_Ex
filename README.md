...
このリポジトリは [ronron-gh/AI_StackChan_Ex](https://github.com/ronron-gh/AI_StackChan_Ex) をフォークし、**「より自然な、生きているようなマスコットロボット」** を目指して独自の拡張を加えています。

### 実装済みの拡張

- **感情・気分システム**: joy / trust / sleepiness / wantToTalk の4パラメータで内面状態を管理。ChatGPT の返答から感情を解析して表情と気分をリアルタイムに更新します。
- **自発発話**: 5分ほど放置すると過去の会話を踏まえて自分から話しかけてきます。
- **就寝・起床システム**: 10分ほど放置すると眠くなり、あくびをして眠りに落ちます。頭をさわると起きてあいさつします。
- **相槌機能**: 音声認識後に感情に合った相槌（「ふむ」「はい」など）を VoiceVox で生成した MP3 で再生します。
- **頭部タッチセンサー**: 長押しで音声認識開始、なでると嬉しそうにします。
- **きょろきょろ**: 待機中にランダムな方向へ頭を向けます。
- **キャラクターシステム**: SD カードの YAML ファイルでキャラクターを定義・切り替え。人格・声・長期記憶の有無・会話スタイルをキャラクターごとに設定できます。
- **長期記憶**: ユーザーについて知ったことを記憶し、再起動後も引き継いで成長します。
- **ブラウザ設定 UI**: スタックちゃんの IP アドレスにブラウザからアクセスしてキャラクター設定・サーボオフセットを変更できます。

詳細は [`my_doc/FeatureMemo.md`](my_doc/FeatureMemo.md) を参照してください。

### M5Stack 公式スタックちゃんでの動作確認

M5Stack 公式スタックちゃん（CoreS3 SE + SCS0009 サーボ）での動作を確認しています。`SC_BasicConfig.yaml` を以下のように設定してください。
...
servo:
  pin:
    ...
    y: 6    # G6 = Servo_TX
  center:
    x: 150  # SCS0009 の物理的中心（300度範囲の中央）
    y: 90
servo_type: "M5_SCS"  # PY32IOExpander 経由の半二重通信
```
...
| 機能 | 概要 |
|------|------|
| 夢の発話 | 就寝時に会話ログを LLM に渡して夢を生成し、起床時に話す |
| 音源定位・顔追従 | デュアルマイクで音の方向を推定して顔を向け、カメラで見つめる |
| 表情ランダム演出 | アイドル時に気分に応じた表情をランダムに切り替える |
| 頭部 LED 感情可視化 | joy / trust を頭部 LED の色・輝度でリアルタイム表示 |
| ベクトル検索長期記憶 | 意味的に検索できる高精度な長期記憶システム |
