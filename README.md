...
- **感情管理 (`StackChanMind`)**: スタックちゃんの感情状態（happy / neutral / sad / angry / doubt / sleepy）を一元管理するクラスを追加。感情が変わった瞬間に Avatar 表情へ自動反映します。
- **感情連動表情変化**: ChatGPT の返答末尾に `[META]{"emotion":"happy"}[/META]` 形式で次の感情を返させ、発話終了後に表情を切り替えます。
- **相槌機能 (`AckSound`)**: 音声認識（STT）完了直後、LLM の応答を待つ間に感情に応じた相槌 MP3 を自動再生します。VoiceVox で生成した音声ファイルを SD カードに配置して使用します。
- **アイドル時きょろきょろ機能 (`HeadMotionController`)**: 待機中にランダムな方向へ頭を向ける動作を実装。目の動き（Avatar サッカード）とは独立してサーボを直接制御します。将来の動作追加（話中うなずき・顔追従など）はストラテジーパターンで差し替え可能です。
...
| 機能 | 概要 |
|------|------|
| アイドル時の生体感演出（拡張） | 表情変化・顔検出時の微笑みなどをきょろきょろ機能と組み合わせる |
| ウェイクワード後の音源定位・顔追従 | デュアルマイクで方向を推定して顔の方向を向き、見つめる |
| 就寝・起床と夢の発話 | 時刻や眠気パラメータで眠りにつき、その日の出来事をもとにした夢を話す |
| 自律思考・自発発話 | 話題ストックや気分パラメータをもとに、ユーザーの入力なしで話しかける |
