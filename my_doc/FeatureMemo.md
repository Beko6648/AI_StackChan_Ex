...
- [ ] **LLM完了前にTTS生成を開始する（非同期TTS）**: `invokeAsyncTtsStreamTask()` を標準ビルド（ChatGPT + Whisper + VoiceVox）でも有効化する。文章を `。！？` で区切りながら逐次 TTS に送ることで最初の音声が出るまでの時間を短縮できる。実装は既存の仕組みが揃っており、`Robot::init()` に1行追加するだけで有効になる可能性がある。
