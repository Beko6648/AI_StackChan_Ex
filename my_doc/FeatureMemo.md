...
WebAPI エンドポイント `/sleep` で、ユーザーが任意のタイミングで sleep/wakeup を操作可能。

**エンドポイント仕様**
- **URL**: `POST /sleep`
- **パラメータ**: `action` (値: `"sleep"` | `"wakeup"`)
- **レスポンス**: JSON `{"status":"OK","action":"..."}` または エラー

**動作**
- `action=sleep`: sleepiness を 1.0 に設定 → Sleeping 表情 + 就寝音声再生（未実装時のみ）
- `action=wakeup`: sleepiness をリセット（0.0）→ 起床音声再生 + Neutral 表情 + 頭部動作再開
...
### 音声録音 VAD（Voice Activity Detection）

`AudioWhisper`（`firmware/src/driver/AudioWhisper.cpp`）に実装。発話が終わって無音が続いたら自動で録音を打ち切り、Whisper API に送るデータ量を削減してレスポンスを改善。

**タイミング改善実績:** 録音時間 固定7〜10秒 → 発話に応じて2〜4秒程度に短縮

**動作フロー:**
1. DMA 対応内部 RAM（300バイト・`chunk_buf`）に録音 → 振幅をリアルタイム計算
2. PSRAM のメインバッファへコピー（Whisper API 送信用）
3. 発話検出後、無音が約0.94秒続いたら録音終了

**主なパラメータ（`dma_buf_len=1024` 時）:**

| パラメータ | 値 | 意味 |
|---|---|---|
| `record_number` | 600 | 最大録音チャンク数（約9.6秒） |
| `SILENCE_CHUNKS` | 59 | 無音終了判定（約0.94秒） |
| `MIN_SPEECH_CHUNKS` | 20 | VAD 発火の最低チャンク数 |
| `s_silenceThreshold` | 200（初期値） | 無音判定の振幅閾値（適応的に更新） |
| `THRESHOLD_MIN` | 50 | 閾値の下限 |

**適応的閾値:**
録音ごとに環境ノイズを計測して次回の閾値を自動調整する。
- 発話前に無音あり → 発話前の環境ノイズで更新（優先）
- 発話直後に話した場合 → 発話後の無音（VAD 終了区間）で更新
- 発話未検出 → 閾値を半分に下げて感度を上げる
