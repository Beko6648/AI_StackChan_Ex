...
**最新機能：** Claude Code モード・感情表現（META タグ）対応  
...
### 感情表現機能（Meta Tag Parser）
Claude Code モード・ChatGPT モード両方で感情表現が動作する：

1. **LLM からの応答に META タグを含める**
   ```
   "こんにちは！[META]{"emotion":"happy","joy":0.3,"trust":0.1}[/META]"
   ```
   - キャラクター YAML の `system_prompt` 末尾に META タグ形式の指示を追加
   - 感情名：`neutral` / `happy` / `sleepy` / `doubt` / `sad` / `angry`
   - joy / trust：0.0〜1.0 の浮動小数点値

2. **共通関数 `applyMetaTag()`**
   - `firmware/src/MetaTagParser.h/cpp` に実装
   - テキストから META タグを解析し、感情を StackChanMind に適用
   - joy・trust を MoodManager に反映
   - タグをテキストから除去

3. **モード別適用箇所**
   - **ChatGPT モード**：`ChatGPT.cpp` で LLM 応答に対して applyMetaTag()
   - **Claude Code モード**：`receiveCommandResult()` で polling.ps1 からの応答に対して applyMetaTag()
